...
public class FinishStockout implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        //获取已完成出库操作的记录
        List<Map<String, Object>> finishedStockout = sqlutil.getInfoBySqlWithTrayNum("select * from stockout_finish", "stockout_finish");
        if (!finishedStockout.isEmpty()) {
            for (Map<String, Object> m : finishedStockout) {
                ...
                if (wmsStock == null) {
                    sqlutil.execute("delete from stockout_finish where id = " + (int)m.get("id"));
                    continue;
                }
                ...
                sqlutil.delAndInsFinished("stockout_finish", (int) m.get("id"), (int) m.get("tray_number"), (Date) m.get("finish_date"));
            }
