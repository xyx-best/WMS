        ...
        <a-row :gutter="6">
          <a-col :span="12">
            <a-form-item label="入库明细编码" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="[ 'stockindtlCode', validatorRules.stockindtlCode]"
                       placeholder="请输入入库明细编码" :readOnly="disableSubmit"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="入库ID" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="[ 'stockinId', validatorRules.stockinId]" placeholder="请输入入库ID" :readOnly="disableSubmit"></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="6">
          <a-col :span="12">
            <a-form-item label="入库编码" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="[ 'stockinCode', validatorRules.stockinCode]" placeholder="请输入入库编码" :readOnly="disableSubmit"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="入库状态" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-dict-select-tag type="list" v-decorator="['stockinState']" :trigger-change="true"
                                 dictCode="stockin_state" placeholder="请选择入库状态" :disabled="disableSubmit"/>
            </a-form-item>
          </a-col>
        </a-row>
  ...
  export default {
    ...
    methods: {
      ...
      edit(record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model, 'stockindtlCode', 'stockinId', 'stockinCode', 'stockinState', 'goodsId', 'goodsName', 'goodsSize', 'goodsUnit', 'goodsType', 'goodsColor', 'goodsBatchnumber', 'goodsQuantity', 'goodsLevel', 'createBy', 'createTime', 'updateBy', 'updateTime'))
        });
      },
      close() {
        this.$emit('close');
      ...
      popupCallback(row) {
        this.form.setFieldsValue(pick(row, 'stockindtlCode', 'stockinId', 'stockinCode', 'stockinState', 'goodsId', 'goodsName', 'goodsSize', 'goodsUnit', 'goodsType', 'goodsColor', 'goodsBatchnumber', 'goodsQuantity', 'goodsLevel', 'createBy', 'createTime', 'updateBy', 'updateTime'))
      },
      getFormData(){
        let formdata_arr = []
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.$emit("validateError","入库明细表表单校验未通过");
          }
        })
        console.log("入库明细表表单数据集",formdata_arr);
        return formdata_arr;
      },
